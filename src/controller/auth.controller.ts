...
class AuthController {
  ...
  public register = async (
    ...
    try {
      const userrRegister = await this.authService.register(req.body);
      if (!userrRegister.success) res.status(400).json(userrRegister);
      res.status(200).json(userrRegister);
    } catch (error) {
      _next(error);
    }
  };
  ...
  public renewToken = async (
    ...
    try {
      const authRenewToken = await this.authService.refreshAccessToken(
        refreshToken
      );
  ...
  public logout = async (
    ...
    try {
      ...
      const logoutUser = await this.authService.logout(
        requestUser.refreshToken
      );
      if (!logoutUser.success)
        return res.status(400).json({ success: false, msg: "Can not logout!" });
